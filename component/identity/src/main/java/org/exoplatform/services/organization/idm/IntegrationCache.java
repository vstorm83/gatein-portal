...
public class IntegrationCache extends AbstractInfinispanCacheProvider {
     ...
     * @param list
     */
    void putGtnUserLazyPageList(String ns, Query query, IDMUserListAccess list, UserStatus status) {
        Fqn nodeFqn = getFqn(ns, USER_QUERY_NODE, getQueryKey(query, status));
        ...
        if (ioNode != null) {
            ...
            if (log.isTraceEnabled()) {
                ...
                log.trace(this.toString() + "GateIn user query list cached. Query: " + getQueryKey(query, status) + ";namespace=" + ns);
            }
        }
    }
     ...
     * @return LazyPageList
     */
    IDMUserListAccess getGtnUserLazyPageList(String ns, Query query, UserStatus status) {
        ...
        if (node != null) {
            ...
            if (log.isTraceEnabled() && list != null) {
                log.trace(this.toString() + "GateIn user query list found in cache. Query: " + getQueryKey(query, status)
                        + ";namespace=" + ns);
            }
    ...
    String getQueryKey(Query query, UserStatus status) {
        ...
        sb.append(query.getEmail()).append(SEP).append(query.getFirstName()).append(SEP).append(query.getLastName())
                ...
                .append(query.getToLoginDate()).append(SEP).append(status.name()).append(SEP);
