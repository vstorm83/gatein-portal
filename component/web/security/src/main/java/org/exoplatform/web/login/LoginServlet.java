    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (status == AUTHENTICATED) {
            ...
            if(ssoHelper.skipJSPRedirection() && meetDisabledUser) {
                ...
                if (meetDisabledUser) {
                    String errorData = meetDisabledUser ? new ErrorData(ErrorData.DISABLED_USER_ERROR, disabledUser).toString() : "";
                    loginPath.append("?").append(ErrorData.ERROR_PARAM).append("=").append(URLEncoder.encode(errorData, "UTF-8"));
                }
